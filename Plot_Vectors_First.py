...
First_xy = np.zeros((df.shape[0]-k+1,df.shape[1]-k+1,2))
...
for i in range(df.shape[0]-k+1):
    for j in range(df.shape[1]-k+1):
        ...
        First_xy[i][j][0] = Beta[3]
        First_xy[i][j][1] = Beta[4]
...
for i in range(120):
    for j in range(120):
        if i%2 == 0 and j%2 == 0:
            ax.annotate("", xy=(i-First_xy[i][j][1]*times, j-First_xy[i][j][0]*times), xytext=(i, j),arrowprops=dict(arrowstyle="->"))
